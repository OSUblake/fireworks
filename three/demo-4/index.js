...
// 			precision mediump int;
	...
	void main() {
		...
		// gl_FragColor = vec4(color.rgb * position, 1.0);
		// gl_FragColor = vec4(mix(color.rgb, position, 1.0), 1.0);
...
class Application {
	...
	createParticles() {
		...
		for ( var i = 0; i < 10000 * 3; i ++ ) {
			...
			endPositions.push( (Math.random() - 0.5) * scale );
			endPositions.push( (Math.random() - 0.5) * scale );
			endPositions.push( (Math.random() - 0.5) * scale);
